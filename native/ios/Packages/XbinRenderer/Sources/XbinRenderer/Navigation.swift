...
struct NavView: View {
    ...
    var body: some View {
        ...
        NavigationStack(path: path) {
            ...
            .environment(\.xbinNav, XbinNavFlags(inNavigation: true, pushed: false, inSheet: inSheet))
            .navigationDestination(for: String.self) { key in
                if let screen = cx?.model.node(key) {
                    NodeView(node: screen)
                        .environment(\.xbinNav, XbinNavFlags(inNavigation: true, pushed: true, inSheet: inSheet))
                }
            }
        }
...
/// `split`: list/detail. Two columns (`NavigationSplitView`) when the width
/// is regular and `prefer` isn't `single`; stacked otherwise (§15).
/// `ArrangementView` (iOS 27.1, the iPhone Duo) is for later: its names are
/// to be confirmed against the SDK first (native/AGENTS.md).
struct SplitView: View {
    ...
    var body: some View {
        ...
        if kids.count >= 2 && width == .regular && node.props.string("prefer") != "single" {
            NavigationSplitView {
                NodeView(node: kids[0]).environment(\.xbinNav, flags)
            } detail: {
                NodeView(node: kids[1]).environment(\.xbinNav, flags)
            }
