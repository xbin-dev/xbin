...
// out. Without this, a file larger than the 16 KiB tool-result cap could only
// ever be read middle-elided — and the elision message tells the model to read
// a range, so there has to be a range to read.
func sliceLines(content string, offset, limit int) string {
	...
	if start >= len(lines) {
		return fmt.Sprintf("(offset %d is past the end — the file has %d lines)", offset, len(lines))
	}
	end := len(lines)
