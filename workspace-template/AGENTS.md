...
component's own directory and `$HOME`** — everything else it sees is
read-only. What it SEES follows the driving user's access (D40): tiles you
hold `read` on are mounted read-only (read siblings for deps/patterns and
API integration); tiles below your read level are simply absent — contents
AND names. Workspace root files (`xbin.json`, `go.work`) are redacted/
filtered to what you can read. The platform's secrets are **not even
present**: `.xbin/` (tokens), `data/` (vault, resource state), and other
users' `homes/` are not mounted — use the resource/vault APIs, not raw
...
its corner, and its body **scrolls internally** — content can never stretch the
card. A tile can also be opened full page. Tiles resize down to **~192px wide**
and default to ~576×384, so design every tile to be usable when **narrow** and
...
- **Reaching a person who is away** is a different channel: `xbin.NotifyUser(ctx,
  user, title, body, link)` (Go SDK; any backend runtime: `POST /api/xbin/notify`)
  pushes to that person's phone through the xbin app — only people who can
  read your tile, from your backend, rate-limited, muteable per tile. Use it for what they must act on (a
