...
- **`composer`**: bind `value` with `@input`, send on `@send {value}`;
  ...
  in it becomes the file's name, percent-encoded, and the path may carry a
  query (e.g. `/api/<self>/ask/upload?draft=…&name={name}`); `method` is
  `PUT` (the default), `POST` or `PATCH`. A JPEG, PNG or GIF photo arrives
  as it is; other photos (HEIC, …) and camera shots arrive as JPEG, videos
  as the movie file (`.mov`, `.mp4`). A file over 64 MiB is not sent (nor
  read): you still get `@uploaded` for it, with `response` `{error, status:
  413}`, and the app tells the user.
...
- the runtime crashes, or native views are switched off: by the user (the
  app's Settings), for the whole workspace by an admin (admin console →
  workspace → xbin app; `whoami` then says `native: {runtime: 0}`, see
  [elements.md](/docs/elements.md) §Native app UI — an app showing the
  workspace follows the switch at once), or for an app build with a known
  problem (the app's remote switch — so a bad app release falls back to web
