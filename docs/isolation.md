...
buxon runs on two planes, and **both** are sandboxed with a default-deny
...
workspace `home/`, `data/`, `.buxon/`, or the host — they simply aren't
...
- **The gateway.** Reaching buxond and calling other components (subject to your
  grants) goes over the gateway unix socket in env as `BUXON_GATEWAY` — that is
...
- **Component terminal** (opened on a component) — the workspace is mounted
  **read-only except `$HOME` and that component's own directory**. You can read
  ...
  (`buxon.json`, `AGENTS.md`, `go.work`), `data/`, or `.buxon/`.
...
buxond upgrades** (they're workspace data, not part of any rootfs).
...
captured in a **persistent per-component layer** at `.buxon/term/<component>/`
...
- **The terminal dev layer** (`.buxon/term/…`, above) is for *interactive* work:
...
- **internet** *(default)* — its own network namespace with an egress relay that
  ...
  `BUXON_URL` is transparently routed so `bx`/`curl` still reach buxond.
