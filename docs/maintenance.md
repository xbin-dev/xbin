...
| `node --test hack/*.test.mjs` — unit tests for pure frontend modules | `js-test` | the shell's context-menu builders (`shell/menus.js`), revisioned-draft helpers (`shell/rev-draft.js`), grid math (`shell/grid-layout.js` — the push a drag performs) and the terminal's prediction engine (`web/term-predict.js` — what a keystroke predicts, what an ack confirms): every branch a menu can show, how a stale save is classified, where a pushed tile lands, which predictions survive — without a browser |
