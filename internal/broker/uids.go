...
// dedicated uid allocated from uidBase. Requires buxond to stay root
// (--scope-uids). Effects:
...
//
// The uid→scope mapping persists in .buxon/uids.json so uids stay stable
...
func newUIDAllocator(root string) (*uidAllocator, error) {
	a := &uidAllocator{
		path:  filepath.Join(root, ".buxon", "uids.json"),
		next:  uidBase,
