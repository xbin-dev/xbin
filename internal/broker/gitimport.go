...
func (b *Broker) unresolvedUses(comp string) []string {
	...
	for _, u := range c.Manifest.Uses {
		...
		case strings.HasPrefix(t, "net:"), strings.HasPrefix(t, "gpu:"), strings.HasPrefix(t, "cap:"),
			t == "code", t == "xbin", strings.HasPrefix(t, "xbin:"):
			// capability targets — always resolvable ("code" = read-all-source;
			// cap:* / xbin* are reserved names, never component paths)
